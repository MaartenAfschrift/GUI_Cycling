function [vcerel] = Muscle_F_V_VU_v2(fcerel,q,lcerel,parms,varargin)
...
lcereld = if_else((fcerel./q)<=fisomrel,brel.*(fcerel-q.*fisomrel)./(fcerel+q.*arel),...
    (-(fcerel + p3 + p1.*p4 + (fcerel.^2 - 2*fcerel.*p1.*p4 + ...
    2*fcerel.*p3 + p1.^2.*p4.^2 - 2*p1.*p3.*p4 + p3.^2 + 4*p2.*p4).^(1/2))./(2.*p4)));
vcerel = if_else(dvdf_isom_ecc>(sloplin./slopfac),(sloplin./slopfac).*(fcerel-q.*fisomrel),lcereld);
...
% sigma = @(x,x0,w) 1./(1+exp(-(x-x0)/w));% zelf gedaan ?
% 
